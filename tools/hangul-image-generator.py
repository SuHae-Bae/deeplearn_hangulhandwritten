...
# mnist 데이터 실습 때와 같이 28*28 사이즈로 해보려고했으나
# 그러면 이미지가 짤려서 안 됨. 64*64 로 해야 제대로 글자가 나옴
IMAGE_WIDTH = 64
IMAGE_HEIGHT = 64
...
def generate_hangul_images(label_file, fonts_dir, output_dir):
    ...
    labels_csv = io.open(os.path.join(output_dir, 'labels-map.csv'), 'w', encoding='utf-8-sig')
    ...
    for character in labels:
        ...
        for font in fonts:
            ...
            file_path1 = os.path.join('./image-data/hangul-images', file_string)
            image.save(file_path, 'JPEG')
            labels_csv.write(u'{},{}\n'.format(file_path1, character))
